...
from covcurv.reads import BamReadsProcessor, cigar_segment_bounds
...
def bam_setup(request):
    ...
    bam_file_single = os.path.join(THIS_DIR, 'data', 'T2_FF_small.bam')  # sample single-end reads data
    bai_file_single = os.path.join(THIS_DIR, 'data', 'T2_FF_small.bai')  # corresponding .bai index file
...
# need .gtf files in order to test coverage parsing.
# (GeneAnnotationLoader should have already been tested.)
@pytest.fixture
def genes_paired_setup(request):
    gtf_file = os.path.join(THIS_DIR, 'data', 'human_chr1.gtf')
...
@pytest.fixture
def genes_single_setup(request):
...
def test_bam_load_single(bam_setup):
    ...
    for chrom in ['chr4', 'chr7']:
        read_count_df = bam_setup.load_chromosome_reads(chrom)
...
def test_bam_coverage_counts_paired(bam_setup, genes_paired_setup):
    bam_setup = bam_setup[0]
    exons_df = genes_paired_setup
...
def test_bam_coverage_counts_single(bam_setup, genes_single_setup):
    bam_setup = bam_setup[1]
    exons_df = genes_single_setup
    fps, read_count_df = bam_setup.coverage_read_counts(exons_df)
    assert isinstance(read_count_df, DataFrame)
    assert read_count_df.shape[1] == 3
    assert all([os.path.isfile(x) for x in fps])
...
# ----------------------------------------------------- #
# Other covcurv.reads module tests
# ----------------------------------------------------- #
